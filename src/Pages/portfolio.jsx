import React from "react";
import Header from "../components/header";
import { Link } from "react-router-dom";
import OtaghBazargani from "../img/OtaghBazargani.png";
import PigGame from "../img/PigGame.png";
import VidlyApp from "../img/VidlyApp.png";

const Portfolio = () => {
  return (
    <>
      <div className="px-0 py-5 m-0 position-relative top-0 left-0 w-100 ">
        <div className="container row mx-auto d-flex align-items-center">
          <div className="col col-md">
            <div className="row resumeSection p-5">
              <h6
                className="p-5 rtl w-75 mx-auto text-center"
                style={{ lineHeight: "2rem" }}
              >
                این پروژه با استفاده از HTML و CSS و jQuery خام ساخته شده است و
                به صورت تک صفحه ای اجرا شده است. برای دیدن پروژه روی عکس کلیک
                کنید
              </h6>
            </div>
          </div>
          <div className="col-md d-none p-4 d-md-flex justify-content-end p-0">
            <a
              href="https://mjjahangiri.github.io/Otagh-Bazargani/"
              target="_blank"
            >
              <img className="img" src={OtaghBazargani} width="100%" alt="" />
            </a>
          </div>
        </div>
        <hr className="text-dark w-50 mx-auto fs-2" />
        <div className="container row mx-auto py-3 d-flex align-items-center">
          <div className="col-md d-none p-4 d-md-flex justify-content-start p-0">
            <a href="https://mjjahangiri.github.io/piggame/" target="_blank">
              <img src={PigGame} className="img" width="100%" alt="" />
            </a>
          </div>
          <div className="col col-md p-4">
            <div className="row resumeSection">
              <h6
                className="p-5 rtl w-75 mx-auto text-center"
                style={{ lineHeight: "2rem" }}
              >
                این بازی با استفاده از JavaScript و Bootstrap پیاده سازی شده
                است. برای دیدن بازی روی عکس کلیک کنید
              </h6>
            </div>
          </div>
        </div>
        <hr className="text-dark w-50 mx-auto fs-2" />
        <div className="container row py-3 mx-auto d-flex align-items-center">
          <div className="col col-md p-4">
            <div className="row resumeSection">
              <h6
                className="p-5 rtl w-75 mx-auto text-center"
                style={{ lineHeight: "2rem" }}
              >
                این پروژه با استفاده از React و Bootstrap پیاده سازی شده است و
                برای قسمت بک اند در localhost از پکیج Json-server استفاده شده تا
                بتوانیم عملیات CRUD و احراز هویت راانجام دهیم. برای دیدن پروژه
                روی عکس کلیک کنید.
              </h6>
            </div>
          </div>
          <div className="col-md d-none p-4 d-md-flex justify-content-end p-0">
            <a href="https://mjjahangiri.github.io/vidly/" target="_blank">
              <img src={VidlyApp} className="img" width="100%" alt="" />
            </a>
          </div>
        </div>
      </div>
    </>
  );
};
export default Portfolio;
